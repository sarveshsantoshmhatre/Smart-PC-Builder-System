...
function aiBuildContext() {
  const b = state.build;
  if (!b) return "No build has been generated yet.";
  return JSON.stringify({
    userRequirements: {
      budgetINR: b.budget,
      workload: b.workload,
      resolution: b.resolution,
      cpuPreference: $("cpuPreference").value,
      ramTargetGB: selectedRamNeed(),
      storageTargetTB: selectedStorageNeed()
    },
    recommendedBuild: {
      cpu: { name:b.cpu.name, priceINR:b.cpu.price, socket:b.cpu.socket, cores:b.cpu.cores, powerW:b.cpu.power },
      gpu: { name:b.gpu.name, priceINR:b.gpu.price, vramGB:b.gpu.vram, performanceScore:b.gpu.score, aiScore:b.gpu.ai, powerW:b.gpu.power },
      motherboard: { name:b.motherboard.name, priceINR:b.motherboard.price, socket:b.motherboard.socket, memoryType:b.motherboard.ramType },
      ram: { name:b.ram.name, priceINR:b.ram.price, capacityGB:b.ram.gb, type:b.ram.type },
      storage: { name:b.storage.name, priceINR:b.storage.price, capacityTB:b.storage.tb },
      psu: { name:b.psu.name, priceINR:b.psu.price, capacityW:b.psu.watts },
      cooler: { name:b.cooler.name, priceINR:b.cooler.price },
      case: { name:b.case.name, priceINR:b.case.price, gpuClearanceMM:b.case.gpuClearance }
    },
    estimatedTotalINR: b.total,
    optimizationScore: b.score,
...
function setAiStatus(connected) {
  ...
  el.innerHTML = connected ? "<i></i> Gemini connected" : "<i></i> Not connected";
}
...
function setAiLoading(loading) {
  ...
  btn.querySelector("span").textContent = loading ? "Thinking…" : "Ask Gemini";
}

function buildAiPrompt(question) {
  const history = aiHistory.slice(-6).map(x => (x.role === "user" ? "User: " : "Assistant: ") + x.text).join("\n");
  return `You are the AI Recommendation Assistant inside a Smart PC Builder web application.

Your job is to help the user reason about a PC build using the provided deterministic recommendation output as your factual starting point.

Rules:
- Give practical, technically grounded advice for PC component selection.
- Explain trade-offs rather than pretending there is one universal answer.
- Respect the user's budget in INR.
- Distinguish the current recommended build from optional alternatives.
- Do not claim live retailer prices or current availability.
- Do not invent compatibility that is not supported by the supplied component data.
- For AI/ML workloads, discuss VRAM, CUDA/ecosystem considerations, RAM, storage, and CPU balance where relevant.
- For gaming, discuss resolution, GPU/CPU balance, and upgrade headroom where relevant.
- For Python/TensorFlow/local AI questions, explicitly call out the practical implications of GPU VRAM and software ecosystem.
- Keep responses concise but useful. Use clear headings and short bullet points when helpful.
- When suggesting a change, state the reason and the budget implication when the catalog gives enough information.
...
async function askGemini(question) {
  if (!sessionGeminiKey) {
    ...
    $("geminiKey").focus();
    showToast("Connect a Gemini API key in AI settings first.");
    return;
  }
  ...
  aiHistory.push({ role:"user", text:userMessage });
  $("aiQuestion").value = "";
  setAiLoading(true);
  ...
  try {
    ...
    const response = await fetch(endpoint, {
      ...
      headers: {
        ...
        "x-goog-api-key": sessionGeminiKey
      },
      body: JSON.stringify({
        systemInstruction: {
          parts: [{ text: "Return plain-text answers suitable for rendering inside a chat panel. Do not include raw HTML." }]
        },
        contents: [{ role: "user", parts: [{ text: buildAiPrompt(userMessage) }] }],
        generationConfig: {
          temperature: 0.35,
          maxOutputTokens: 900
        }
    ...
    if (!response.ok) {
      const detail = data?.error?.message || `Gemini request failed with HTTP ${response.status}`;
    ...
    appendChat("assistant", `I couldn't complete the AI request. ${message}`);
    aiHistory.push({ role:"assistant", text:`AI request failed: ${message}` });
    showToast("Gemini request failed. Check the model and API key.");
  } finally {
    setAiLoading(false);
  }
}
...
$("aiSettingsBtn").addEventListener("click", () => {
  $("aiSettings").hidden = !$("aiSettings").hidden;
});
...
$("connectAiBtn").addEventListener("click", () => {
  ...
  setAiStatus(true);
  $("aiSettings").hidden = true;
  showToast("Gemini connected for this browser session.");
});

$("forgetAiBtn").addEventListener("click", () => {
  sessionGeminiKey = "";
  aiHistory = [];
  setAiStatus(false);
  showToast("Gemini session key cleared.");
});

document.querySelectorAll(".ai-quick button").forEach(btn => {
  btn.addEventListener("click", () => askGemini(btn.dataset.question));
});

$("aiForm").addEventListener("submit", event => {
  event.preventDefault();
  askGemini($("aiQuestion").value);
});

$("aiQuestion").addEventListener("keydown", event => {
  if (event.key === "Enter" && (event.ctrlKey || event.metaKey)) {
    event.preventDefault();
    askGemini(event.currentTarget.value);
  }
});
